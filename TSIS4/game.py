...
def run_game(screen, settings, personal_best):
    ...
    #Function to generate obstacles based on current level. More obstacles appear at higher levels, but they won't spawn in a safe zone around the starting point to avoid trapping the snake.
    ...
    #Function to get a random position on the grid that is not occupied by the snake or obstacles
    def get_random_pos():
        while True:
    ...
    while running:
        ...
        #Handle events (movement, quitting, etc.)
    ...
    #Return final score and level for leaderboard
    return score, level